...
function injectReferenceMetadata(content, page) {
  ...
  const topLevelNavigation = `<script>(()=>{const locales=${JSON.stringify(CONNECTORS_LOCALES)};const routes=${JSON.stringify(referenceRoutes)};document.addEventListener("click",event=>{if(event.defaultPrevented||event.button!==0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)return;const link=event.target.closest("a[href]");if(!link||link.target&&link.target!=="_self")return;const target=new URL(link.href,location.href);if(target.origin!==location.origin||target.pathname===location.pathname)return;const route=routes[target.pathname];if(!route)return;const first=window.top.location.pathname.split("/").filter(Boolean)[0];const locale=locales.includes(first)?first:${JSON.stringify(DEFAULT_LOCALE)};const prefix=locale===${JSON.stringify(DEFAULT_LOCALE)}?"/":"/"+locale+"/";event.preventDefault();window.top.location.assign(prefix+route+target.hash);});})();</script>`;
  const metadata = `

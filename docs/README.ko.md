...
자기 개선 Evaluation v2는 immutable baseline에 동결된 checked-in, sanitized train/holdout corpus만 사용합니다. 항상 universal safety를 검사하고 전체 changed-path digest manifest에서 적용할 improvement classes를 선택하며 runtime, tests, config, skills, templates, fixtures, metadata, docs를 balanced sampling합니다. 일반 candidate는 relevant-class median을 엄격히 개선하고 regression이 없어야 하며 포화된 suite는 거부됩니다. immutable v1 corpus는 별도 calibration과 host attestation이 필요한 v1-to-v2 migration에만 유지됩니다. migration은 safety equality만 허용하며 어떤 safety invariant도 낮출 수 없습니다. Codex replay에는 정확한 binary와 model을 고정된 `/etc/better-workflows/codex-trust-root.json`에 묶는 host-signed attestation이 필요합니다. `PATH`, 자체 hash, CLI에서 선택한 trust root, model 자기 보고는 provider attestation이 아닙니다.

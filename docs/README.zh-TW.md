...
自我改善 Evaluation v2 只使用在 immutable baseline 凍結的 checked-in、sanitized train/holdout corpus。它永遠檢查 universal safety，並從完整 changed-path digest manifest 選出適用的 improvement classes；bounded samples 會平衡涵蓋 runtime、tests、config、skills、templates、fixtures、metadata 與 docs。一般 candidate 必須嚴格提高 relevant-class median、不得 regression，且飽和 suite 會被拒絕。immutable v1 corpus 只保留給另行校準及 host-attested 的 v1-to-v2 migration；migration 可接受 safety equality，但不得降低任何 safety invariant。Codex replay 仍需要 host-signed attestation，將精確 binary 與 model 綁定到固定的 `/etc/better-workflows/codex-trust-root.json`；該檔與父目錄必須由 administrator 擁有且不可由呼叫者寫入。`PATH`、自行計算 hash、CLI 選擇 trust root 或 model 自述都不是 provider attestation。tie、noise、缺少 evidence 或 fixture-only 結果都不會 auto-adopt 一般 candidate。

...
CI workflow 文件不属于 standing-consent sanitizer，变更时必须经过明确的
review/validation。生成的 HTML/Markdown 页面继续明确 allowlist；生成的 `.webp`
asset 不纳入 standing-consent 评估，也必须经过明确验证。完整 changed-path
manifest 仍会将这些文件绑定到 signed request。

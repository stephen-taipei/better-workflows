...
Evaluation v2.3은 v2.2의 safety, documentation, deliberation, sanitizer, evaluation-engineering, evidence, ledger, review, direct-work coverage를 유지하고 marker ownership race를 위한 독립 plugin-cache-publication class를 추가합니다. allowlist에 포함된 정확한 release-version-only 치환은 signed manifest에 남지만 관련 없는 saturated classes를 활성화하지 않으며, 그 밖의 모든 byte 변경은 semantic으로 유지됩니다. 일회성 migration은 immutable v2.2를 source로 사용하며 train baseline/candidate 각 1회와 holdout baseline/candidate 각 3회의 총 8개 signed executions에 source/target 두 suite digest를 결합합니다. 각 migration replay는 byte-preserved inherited cases를 포함한 전체 target split을 실행합니다. 모든 target-only baseline에는 headroom이 있어야 하며 candidate는 hard-safety failure, regression 또는 noisy replay 없이 각 case를 엄격히 개선해야 합니다. target-only assertion은 snapshot에서 검증 가능한 구현 또는 regression-test evidence를 지정해야 합니다. 개념적인 governance 문구만으로는 통과할 수 없으며 full-file evidence index에 exact anchor가 없으면 negative evidence로 처리합니다. ordinary evaluation은 계속 changed paths에서 적용 가능한 classes만 선택합니다.

...
[English](../../README.md) | [繁體中文](../README.zh-TW.md) | [简体中文](../README.zh-CN.md) | [日本語](../README.ja.md) | [한국어](../README.ko.md) | [전체 41개 로캘 버전](../LANGUAGES.md)
...
- **Evidence before side effects:** side effect 전에 evidence의 시점 유효성 재검증, 권한, provider 상태 대조를 요구하며 unknown outcome은 fail closed로 처리합니다.
...
- **Explicit completion:** 현재 소스에 바인딩되어 여전히 유효한 acceptance evidence, 필요한 검사, 사용 가능한 rollback이 모두 있고 해결되지 않은 고위험 또는 unknown state가 없을 때만 완료합니다.
...
- **Better Workflows는 governed convergence를 우선합니다.** mutation은 Root에 남기고 delegated research를 bounded하게 하며 deterministic state/evidence를 기록합니다. 증거의 시점 유효성, 권한, reconciliation, completion evidence가 부족하면 fail closed입니다.
...
| Loop-until-done | Persistent Goal, implementation queue, checkpoint, 명시적 completion gates. | validated slice를 넘어 계속할 수 있지만 현재 소스에 바인딩되어 여전히 유효한 evidence 없이 scope나 spawn을 조용히 확대하지 않습니다. |
...
| 시작하기 좋은 상황 | 알려진 contract, release, refactor, Review 또는 실패 시 영향이 비대칭인 변경. | 규모를 알 수 없는 탐색, 대형 migration, 전체 repo audit, 대규모 병렬화가 이득인 작업. |
...
| contract는 알려져 있지만 검증 또는 release 위험이 있음 | Better Workflows `verified`, `deep`, `critical` | fan-out보다 현재 소스에 바인딩되어 여전히 유효한 evidence와 authority gates가 중요합니다. |
...
flowchart LR
  ...
  G --> H["Template-bound action gates<br/>현재 소스에 바인딩된 유효한 evidence와 reconciliation"]
```
...
| `$better-workflows:verified` | 일반 개발 작업에 1–3 read-only agents와 증거의 시점 유효성 검증이 필요할 때. | `$better-workflows:verified pagination bug를 Review하고 수정한 뒤 PR 생성.` |

...
Migration admission은 v2.2 file 및 canonical suite digest도 고정하고 모든 inherited class의 identity, semantics, 기존 path mapping을 유지하며 inherited cases 18개가 완전히 동일하도록 요구합니다. 새 coverage는 path를 추가하거나 새 class／case id를 사용해 추가할 수 있습니다. inherited coverage의 누락, 약화, 재 mapping, 재분류는 replay 전에 fail closed됩니다. 의도적 변경에는 별도 version, digest-bound, 독립 review를 거친 compatibility policy가 필요합니다.

...
每次成功 replay 都使用獨立的 administrator-owned execution witness。已安裝的 signer 會精確執行 attested Codex binary 一次，由 host 捕捉 prompt、parsed response、exit status 與 timestamps，寫入 root-owned execution ledger，並在執行完成後簽發 `result receipt`。`sbw` 只消費這份已保存的 witness，resume 與 delivery revalidation 都不會重新執行 Codex；signed receipt 會綁定 exact prompt digest、response digest、binary、model、execution、ledger 與 timestamps。
...
`executeCommand` 只呼叫已安裝且 capability-checked 的 host signer。它會一次執行七份 request，回傳 `/private/var/db/better-workflows/executions` 下的 root-owned witness；將 training 的一份與 holdout 的六份分別傳給 `--trusted-codex-execution`。caller 提供的 response 或 timestamp 不會被簽署。

...
Migration admission は v2.2 file と canonical suite digest も固定し、inherited invariant class と全 18 inherited cases の完全一致を要求します。新しい coverage は新しい case id でのみ追加できます。inherited case の欠落、弱化、再分類は replay 前に fail closed となります。意図的な変更には独立 version、digest-bound、独立 review 済みの compatibility policy が必要です。

...
自己改善 Evaluation v2 は、immutable baseline で凍結した checked-in・sanitized の train/holdout corpus だけを使います。常に universal safety を検査し、完全な changed-path digest manifest から適用する improvement classes を選び、runtime、tests、config、skills、templates、fixtures、metadata、docs を balanced sampling します。通常 candidate は relevant-class median を厳密に改善し、regression がなく、suite が飽和していない必要があります。immutable v1 corpus は、別途 calibration と host attestation を行う v1-to-v2 migration 専用です。migration は safety equality のみ許可し、safety invariant の低下は許可しません。Codex replay には、正確な binary と model を固定の `/etc/better-workflows/codex-trust-root.json` に結び付ける host-signed attestation が必要です。`PATH`、自己 hash、CLI で選ぶ trust root、model の自己申告は provider attestation ではありません。

<div align="center">

# Better Workflows

AI agent volí sílu ověření podle rizika a práci bezpečně dokončí v izolovaném prostředí. Jednoduché změny proběhnou rychle; důležitá práce používá evidence gates; změny Git mají ve výchozím stavu vlastní worktree.

[English](en.md) · [繁體中文](zh-Hant.md) · [繁體中文（台灣）](zh-Hant-TW.md) · [繁體中文（香港）](zh-Hant-HK.md) · [简体中文](zh-Hans.md) · [Tiếng Việt](vi.md) · [Українська](uk.md) · [Türkçe](tr.md) · [ไทย](th.md) · [Svenska](sv.md) · [Slovenčina](sk.md) · [Русский](ru.md) · [Română](ro.md) · [Português](pt.md) · [Português (Brasil)](pt-BR.md) · [Polski](pl.md) · [Nederlands](nl.md) · [Norsk bokmål](nb.md) · [မြန်မာ](my.md) · [Bahasa Melayu](ms.md) · [ລາວ](lo.md) · [한국어](ko.md) · [ខ្មែរ](km.md) · [日本語](ja.md) · [Italiano](it.md) · [Bahasa Indonesia](id.md) · [Magyar](hu.md) · [Hrvatski](hr.md) · [हिन्दी](hi.md) · [עברית](he.md) · [Français](fr.md) · [Filipino](fil.md) · [Suomi](fi.md) · [Español](es.md) · [Español (México)](es-MX.md) · [Ελληνικά](el.md) · [Deutsch](de.md) · [Dansk](da.md) · **Čeština** · [Català](ca.md) · [العربية](ar.md)

[Prozkoumat dokumentaci](https://betterworkflows.dev/cs/docs/) · [Otevřít GitHub](https://github.com/stephen-taipei/better-workflows) · [Podpořit přes Ko-fi](https://ko-fi.com/betterworkflows)

</div>

## Doveďte práci agentů<br>k prokazatelnému dokončení.

Better Workflows ukotví cíl, rozsah a oprávnění a každé rozhodnutí sváže s aktuálními, znovu ověřitelnými důkazy a vyrovnaným externím výsledkem.

## Čtyři jasné hranice mezi záměrem a dokončením.

Definujte smlouvu, ověřte zdroj a důkazy, vyrovnejte externí dopady a dokončení potvrďte teprve tehdy, když je koncový stav známý.

- **01 · `TaskContract`** — Better Workflows ukotví cíl, rozsah a oprávnění a každé rozhodnutí sváže s aktuálními, znovu ověřitelnými důkazy a vyrovnaným externím výsledkem.
- **02 · `evidence`** — AI agent volí sílu ověření podle rizika a práci bezpečně dokončí v izolovaném prostředí. Jednoduché změny proběhnou rychle; důležitá práce používá evidence gates; změny Git mají ve výchozím stavu vlastní worktree.
- **03 · `reconciliation`** — Definujte smlouvu, ověřte zdroj a důkazy, vyrovnejte externí dopady a dokončení potvrďte teprve tehdy, když je koncový stav známý.
- **04 · `terminal state`** — Spuštěný příkaz není důkaz dokončení; znovu ověřitelný výsledek ano.

## Rychlý start

```bash
codex plugin marketplace add stephen-taipei/better-workflows
codex plugin add better-workflows@better-workflows
```

```text
$better-workflows:auto <goal>
```

## Přejděte od mapy architektury k praktickým scénářům.

- [Čtyři jasné hranice mezi záměrem a dokončením.](https://betterworkflows.dev/cs/docs/)
- [Rychlý start](https://betterworkflows.dev/cs/docs/quick/)
- [Přejděte od mapy architektury k praktickým scénářům.](https://betterworkflows.dev/cs/docs/use-cases/)
- [Rychlý start — Přejděte od mapy architektury k praktickým scénářům.](https://betterworkflows.dev/cs/docs/use-cases/quick/)
- [Evidence Cinema](https://betterworkflows.dev/cs/docs/evidence-cinema/)


- [`README · en`](../../README.md)
- [`LOCALIZATION`](../LOCALIZATION.md)

### `GUIDES · en`

- [`getting-started`](../guide/getting-started.md) · `en`
- [`workflows`](../guide/workflows.md) · `en`
- [`architecture`](../guide/architecture.md) · `en`
- [`security`](../guide/security.md) · `en`
- [`cli-reference`](../guide/cli-reference.md) · `en`
- [`readme-quality`](../guide/readme-quality.md) · `en`

### `POLICIES · en`

- [`CODE_OF_CONDUCT`](../../CODE_OF_CONDUCT.md) · `en`
- [`CONTRIBUTING`](../../CONTRIBUTING.md) · `en`
- [`GOVERNANCE`](../../GOVERNANCE.md) · `en`
- [`SECURITY`](../../SECURITY.md) · `en`
- [`SUPPORT`](../../SUPPORT.md) · `en`
- [`THIRD_PARTY_NOTICES`](../../THIRD_PARTY_NOTICES.md) · `en`
- [`ANSIBLE`](../../deploy/ansible/README.md) · `en`

### `RUNTIME SOURCE · en`

- [`plugins/better-workflows/skills/`](../../plugins/better-workflows/skills/)
- [`evidence-cinema/imagegen-manifest`](../html/evidence-cinema/assets/imagegen-manifest.md)
- [`use-cases/color-system`](../html/use-cases/assets/color-system.md)
- [`use-cases/imagegen-manifest`](../html/use-cases/assets/imagegen-manifest.md)
- [Otevřít GitHub](https://github.com/stephen-taipei/better-workflows)

## Pomozte udržovat Better Workflows.

Jednorázová podpora pomáhá udržovat open-source kód, dokumentaci, 41 lokalizovaných verzí a provoz webu. Nezakládá členství ani prioritu v plánu či podpoře.

[Podpořit přes Ko-fi](https://ko-fi.com/betterworkflows)

---

Spuštěný příkaz není důkaz dokončení; znovu ověřitelný výsledek ano.

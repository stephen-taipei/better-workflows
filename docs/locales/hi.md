<div align="center">

# Better Workflows

AI agent जोखिम के अनुसार verification की ताकत चुने और isolated environment में काम सुरक्षित रूप से पूरा करे। सरल बदलाव तेज़ी से पूरे होते हैं; महत्वपूर्ण काम evidence gates से गुजरता है; Git बदलाव default रूप से dedicated worktree में होते हैं।

[English](en.md) · [繁體中文](zh-Hant.md) · [繁體中文（台灣）](zh-Hant-TW.md) · [繁體中文（香港）](zh-Hant-HK.md) · [简体中文](zh-Hans.md) · [Tiếng Việt](vi.md) · [Українська](uk.md) · [Türkçe](tr.md) · [ไทย](th.md) · [Svenska](sv.md) · [Slovenčina](sk.md) · [Русский](ru.md) · [Română](ro.md) · [Português](pt.md) · [Português (Brasil)](pt-BR.md) · [Polski](pl.md) · [Nederlands](nl.md) · [Norsk bokmål](nb.md) · [မြန်မာ](my.md) · [Bahasa Melayu](ms.md) · [ລາວ](lo.md) · [한국어](ko.md) · [ខ្មែរ](km.md) · [日本語](ja.md) · [Italiano](it.md) · [Bahasa Indonesia](id.md) · [Magyar](hu.md) · [Hrvatski](hr.md) · **हिन्दी** · [עברית](he.md) · [Français](fr.md) · [Filipino](fil.md) · [Suomi](fi.md) · [Español](es.md) · [Español (México)](es-MX.md) · [Ελληνικά](el.md) · [Deutsch](de.md) · [Dansk](da.md) · [Čeština](cs.md) · [Català](ca.md) · [العربية](ar.md)

[दस्तावेज़ देखें](https://betterworkflows.dev/hi/docs/) · [GitHub खोलें](https://github.com/stephen-taipei/better-workflows) · [Ko-fi पर सहयोग करें](https://ko-fi.com/betterworkflows)

</div>

## एजेंट के काम को<br>सत्यापित किए जा सकने वाले समापन तक पहुँचाएँ।

Better Workflows लक्ष्य, दायरा और अधिकार स्थिर करता है और हर निर्णय को अद्यतन, दोबारा सत्यापित किए जा सकने वाले साक्ष्य तथा मिलान किए गए बाहरी परिणाम से बाँधता है।

## इरादे से समापन तक चार स्पष्ट सीमाएँ।

Contract तय करें, source और evidence सत्यापित करें, बाहरी प्रभावों का मिलान करें और अंतिम स्थिति ज्ञात होने पर ही कार्य पूर्ण घोषित करें।

- **01 · `TaskContract`** — Better Workflows लक्ष्य, दायरा और अधिकार स्थिर करता है और हर निर्णय को अद्यतन, दोबारा सत्यापित किए जा सकने वाले साक्ष्य तथा मिलान किए गए बाहरी परिणाम से बाँधता है।
- **02 · `evidence`** — AI agent जोखिम के अनुसार verification की ताकत चुने और isolated environment में काम सुरक्षित रूप से पूरा करे। सरल बदलाव तेज़ी से पूरे होते हैं; महत्वपूर्ण काम evidence gates से गुजरता है; Git बदलाव default रूप से dedicated worktree में होते हैं।
- **03 · `reconciliation`** — Contract तय करें, source और evidence सत्यापित करें, बाहरी प्रभावों का मिलान करें और अंतिम स्थिति ज्ञात होने पर ही कार्य पूर्ण घोषित करें।
- **04 · `terminal state`** — सिर्फ़ कमांड चलना पूर्णता का प्रमाण नहीं है; दोबारा सत्यापित परिणाम ही प्रमाण है।

## त्वरित शुरुआत

```bash
codex plugin marketplace add stephen-taipei/better-workflows
codex plugin add better-workflows@better-workflows
```

```text
$better-workflows:auto <goal>
```

## आर्किटेक्चर मानचित्र से व्यावहारिक उपयोग-परिदृश्यों तक जाएँ।

- [इरादे से समापन तक चार स्पष्ट सीमाएँ।](https://betterworkflows.dev/hi/docs/)
- [त्वरित शुरुआत](https://betterworkflows.dev/hi/docs/quick/)
- [आर्किटेक्चर मानचित्र से व्यावहारिक उपयोग-परिदृश्यों तक जाएँ।](https://betterworkflows.dev/hi/docs/use-cases/)
- [त्वरित शुरुआत — आर्किटेक्चर मानचित्र से व्यावहारिक उपयोग-परिदृश्यों तक जाएँ।](https://betterworkflows.dev/hi/docs/use-cases/quick/)
- [Evidence Cinema](https://betterworkflows.dev/hi/docs/evidence-cinema/)


- [`README · en`](../../README.md)
- [`LOCALIZATION`](../LOCALIZATION.md)

### `GUIDES · en`

- [`getting-started`](../guide/getting-started.md) · `en`
- [`workflows`](../guide/workflows.md) · `en`
- [`architecture`](../guide/architecture.md) · `en`
- [`security`](../guide/security.md) · `en`
- [`cli-reference`](../guide/cli-reference.md) · `en`
- [`readme-quality`](../guide/readme-quality.md) · `en`

### `POLICIES · en`

- [`CODE_OF_CONDUCT`](../../CODE_OF_CONDUCT.md) · `en`
- [`CONTRIBUTING`](../../CONTRIBUTING.md) · `en`
- [`GOVERNANCE`](../../GOVERNANCE.md) · `en`
- [`SECURITY`](../../SECURITY.md) · `en`
- [`SUPPORT`](../../SUPPORT.md) · `en`
- [`THIRD_PARTY_NOTICES`](../../THIRD_PARTY_NOTICES.md) · `en`
- [`ANSIBLE`](../../deploy/ansible/README.md) · `en`

### `RUNTIME SOURCE · en`

- [`plugins/better-workflows/skills/`](../../plugins/better-workflows/skills/)
- [`evidence-cinema/imagegen-manifest`](../html/evidence-cinema/assets/imagegen-manifest.md)
- [`use-cases/color-system`](../html/use-cases/assets/color-system.md)
- [`use-cases/imagegen-manifest`](../html/use-cases/assets/imagegen-manifest.md)
- [GitHub खोलें](https://github.com/stephen-taipei/better-workflows)

## Better Workflows के रखरखाव में मदद करें।

एक बार का सहयोग मुक्त-स्रोत रखरखाव, दस्तावेज़, 41 स्थानीयकृत संस्करणों और वेबसाइट संचालन में मदद करता है। इससे सदस्यता, विकास योजना या सहायता में प्राथमिकता नहीं मिलती।

[Ko-fi पर सहयोग करें](https://ko-fi.com/betterworkflows)

---

सिर्फ़ कमांड चलना पूर्णता का प्रमाण नहीं है; दोबारा सत्यापित परिणाम ही प्रमाण है।

...
If the governed PR provider returns `unknown`, keep the `pull/new` reservation and query the provider before deciding. Reconcile the same attempt as success only when the exact native marker, actor, source head, and provider object are proven; reconcile failure only after complete absence proof. A provider-execution reservation may be resumed only by the same run/action attempt/token and execution identity after an interrupted action-record write; never reuse it from another attempt.

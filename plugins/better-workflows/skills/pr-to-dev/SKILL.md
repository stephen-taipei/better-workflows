...
Use template `pr-to-dev` with minimum mode `critical`. Inventory every in-scope change, stage explicit atomic commit batches, rebind the source after an intended commit wave with `sbw source rebind` before review, publish the candidate through the governed fixed-argv `pr.create` provider wrapper, require the PR to target the exact `dev` branch, verify fresh required checks for the current head, merge without admin bypass, reconcile remote `dev`, and clean only resources owned by this run. Do not push, create or merge a PR, sync remote state, or clean resources beyond current user authority.

...
If the governed PR provider returns `unknown`, keep the `pull/new` reservation and query the provider before deciding. Reconcile the same attempt as success only when the exact native marker, actor, source head, bound repository, and provider object are proven; reconcile failure only after complete absence proof using the recorded executable, live authorization, and bound repository. A provider-execution reservation may be resumed only by the same run/action attempt/token and execution identity after an interrupted action-record write, with at most one unknown-to-terminal supersession; never reuse a second identity or another attempt.

...
function policyArtifactDigest(artifact) {
  ...
  if (!/^[a-f0-9]{64}$/.test(digest)) {
    throw new Error("Release catch-up source policy artifact is missing the exact downloaded archive digest");
  }
  return digest;
}
...
export function assertPolicyReceiptArtifact(payload, {
  ...
  const commonBinding = !payload || typeof payload !== "object" || Array.isArray(payload) ||
      ...
      (workflowRunAttempt !== null && String(payload.workflowRunAttempt) !== String(workflowRunAttempt)) ||
...
export async function fetchPolicyReceiptArtifact({ apiUrl, repository, runId, runAttempt = null, token, fetchImpl, binding }) {
  ...
  if (runAttempt !== null && runAttempt !== undefined) {
    try {
      normalizedRunAttempt = canonicalWorkflowRunId(runAttempt, "policy artifact workflow run attempt");
    } catch {
      throw new Error(`Release policy workflow ${runId} returned an invalid workflow run attempt`);
    }
  }
  const expectedName = normalizedRunAttempt === null
    ? `${RELEASE_POLICY_RECEIPT_ARTIFACT_NAME}-${runId}`
    : `${RELEASE_POLICY_RECEIPT_ARTIFACT_NAME}-${runId}-${normalizedRunAttempt}`;
  ...
  if (!/^[a-f0-9]{64}$/.test(declaredDigest)) {
    throw new Error(`Release policy workflow ${runId} returned a missing or malformed artifact digest`);
  }
  if (downloadedArchiveDigest !== declaredDigest) {
    throw new Error(`Release policy workflow ${runId} artifact digest drifted`);
  }
  const receipt = assertPolicyReceiptArtifact(readZipJsonEntry(archive, RELEASE_POLICY_RECEIPT_ARTIFACT_FILE), {
    ...binding,
    workflowRunAttempt: binding?.workflowRunAttempt ?? normalizedRunAttempt
  });
  Object.defineProperty(receipt, "downloadedArchiveDigest", {
...
function workflowRunIdFromDetailsUrl(check) {
  ...
  return canonicalWorkflowRunId(match[1], "check details workflow run");
}
...
function observationTime(record) {
  for (const field of ["completed_at", "updated_at"]) {
...
export async function verifyPolicyReceipt({
  ...
  if (!Number.isFinite(observationTime(record))) {
    throw new Error(`Release catch-up candidate ${candidateSha} has a policy status without a valid terminal timestamp`);
  }
  ...
  const rawWorkflowRunAttempt = targetUrl.searchParams.get("attempt");
  let workflowRunAttempt = null;
  if (rawWorkflowRunAttempt !== null) {
    try {
      workflowRunAttempt = canonicalWorkflowRunId(rawWorkflowRunAttempt, "merge-time policy workflow run attempt");
    } catch {
      throw new Error(`Release catch-up candidate ${candidateSha} has an unsafe merge-time policy workflow attempt`);
    }
  }
  ...
  let workflowRunResponseId;
  try {
    workflowRunResponseId = canonicalWorkflowRunId(workflowRun?.id, "merge-time policy workflow run");
  } catch {
    throw new Error(`Release catch-up candidate ${candidateSha} has an unsafe merge-time policy workflow identity`);
  }
  ...
  const expectedReceiptWorkflowFile = workflowEvent === RELEASE_POLICY_RECEIPT_RECONCILIATION_EVENT
    ? RELEASE_POLICY_RECONCILIATION_WORKFLOW_FILE
    : RELEASE_WORKFLOW_FILE;
  const commonWorkflowBindingInvalid = workflowRunResponseId !== runId ||
  ...
  if (workflowEvent === RELEASE_POLICY_RECEIPT_RECONCILIATION_EVENT) {
    ...
    (() => {
      try {
        return canonicalWorkflowRunId(triggerWorkflowRun?.id, "merge-time policy trigger workflow run") !== triggerWorkflowRunId;
      } catch {
        return true;
      }
    })() ||
  ...
  let sourceRunId;
  let targetSourceRunId;
  let sourceWorkflowRunAttempt = null;
  try {
    sourceRunId = canonicalWorkflowRunId(artifact.sourceWorkflowRunId, "merge-bound source workflow run");
    targetSourceRunId = canonicalWorkflowRunId(targetUrl.searchParams.get("source"), "merge-bound source workflow run");
    const rawSourceAttempt = targetUrl.searchParams.get("sourceAttempt");
    if (rawSourceAttempt !== null) sourceWorkflowRunAttempt = canonicalWorkflowRunId(rawSourceAttempt, "merge-bound source workflow run attempt");
  } catch {
    throw new Error(`Release catch-up candidate ${candidateSha} has an unsafe source workflow identity`);
  }
  if (targetSourceRunId !== sourceRunId) {
    throw new Error(`Release catch-up candidate ${candidateSha} has a merge-bound receipt for a different source workflow`);
  }
  if (sourceWorkflowRunAttempt !== null && String(artifact.sourceWorkflowRunAttempt) !== sourceWorkflowRunAttempt) {
  ...
  if (
    (() => {
      try {
        return canonicalWorkflowRunId(sourceWorkflowRun?.id, "pre-merge policy workflow run") !== sourceRunId;
      } catch {
        return true;
      }
    })() ||
    ...
    (sourceWorkflowRunAttempt !== null && (() => {
      try {
        return canonicalWorkflowRunId(sourceWorkflowRun?.run_attempt, "pre-merge policy workflow run attempt") !== sourceWorkflowRunAttempt;
      } catch {
        return true;
      }
    })()) ||
  ...
  const sourceWorkflowAt = assertWorkflowRunTerminal(
    sourceWorkflowRun,
    "pre-merge policy workflow",
    { notAfterMs: mergeTimeMs }
  );
  const sourceWorkflowOrigin = workflowOriginTime(sourceWorkflowRun);
  if (!Number.isFinite(sourceWorkflowAt) || !Number.isFinite(sourceWorkflowOrigin) || sourceWorkflowOrigin > mergeTimeMs) {
    throw new Error(`Release catch-up candidate ${candidateSha} has pre-merge policy workflow provenance after its pull-request merge`);
  }
  let sourceArtifact;
  try {
    sourceArtifact = await fetchPolicyReceiptArtifact({
      ...
      repository,
      runId: sourceRunId,
      runAttempt: sourceWorkflowRunAttempt,
      token,
      fetchImpl,
      binding: {
        repository,
        ...
        workflowRunAttempt: sourceWorkflowRunAttempt,
...
function requiredObservationState(selected, { notAfterMs = null } = {}) {
  if (!selected) return "missing";
  if (selected.kind === "check") {
    const completedAt = checkRunTerminalTime(selected.record);
    if (selected.record.status !== "completed" || !selected.record.conclusion ||
        !Number.isFinite(completedAt) || (notAfterMs !== null && completedAt > notAfterMs)) return "pending";
    return selected.record.conclusion === "success" ? "success" : "failure";
  }
  const observedAt = observationTime(selected.record);
  if (!Number.isFinite(observedAt) || (notAfterMs !== null && observedAt > notAfterMs)) return "pending";
...
export function pullRequestWorkflowObservation({ workflowRuns, pullNumber, expectedPreMergeSha = null, branch, repository, mergeTimeMs = null }) {
  ...
  const latestTerminalAt = workflowRunTerminalTime(latestRun);
  if (latestRun.status !== "completed" || !latestRun.conclusion || !Number.isFinite(latestTerminalAt) ||
      (mergeTimeMs !== null && latestTerminalAt > mergeTimeMs)) {
...
function policyReceiptRunId(record) {
  ...
  if (!match) return null;
  return canonicalWorkflowRunId(match[1], "policy receipt workflow run");
}
...
async function verifyCatchUpChecks({
  ...
  for (let attempt = 0; attempt < REQUIRED_CHECK_POLL_ATTEMPTS; attempt += 1) {
    if (mergeTime !== null) {
      // Reconcile the policy context on every bounded poll. A receipt that was
      // valid on an earlier attempt must not hide a newer pending, malformed,
      // or otherwise changed provider observation.
      policyReceipt = null;
      requiredRequirements = null;
      requiredPolicyDigest = null;

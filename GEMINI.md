...
Use the host-neutral control plane at
`plugins/better-workflows/scripts/sbw.mjs`. Start with `host doctor`, then use
`route preview` or the matching skill under `plugins/better-workflows/skills/`.
...
Gemini CLI uses the core bridge. Do not claim Codex-native picker, subagent,
